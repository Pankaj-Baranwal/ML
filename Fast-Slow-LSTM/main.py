...
# Contains Auxilary functions
import aux
# Contains utilities for parsing PTB text files
import reader
# LSTM and PTB Configurations
import configs
...
flags.DEFINE_string("save_path", '/output/models/',
...
def main(_):
    ...
    with open('/output/output.txt', 'ab+') as f:
        pickle.dump(test_data, f)

        # A new graph will be generated with the following scope
        # Helps when analysing the various vars and ops
        with tf.Graph().as_default():
            # generates tensors with a uniform distribution.
            # Args: minval (lower bound), maxval (upper bound), seed
            initializer = tf.random_uniform_initializer(-config.init_scale,
                                                        config.init_scale)

            # Similar to variable_scope. But variables can be accessed outside scope here using get_variable.
            ...
            with tf.Session() as session:
                ...
                for i in range(config.max_max_epoch):
                    ...
                    sys.stdout.flush()
                    var_a = "Epoch: %d Learning rate: %.3f" % (i + 1, session.run(m.lr))
                    pickle.dump(var_a, f)
                    var_a = "Epoch: %d Train BPC: %.4f" % (i + 1, train_perplexity)
                    pickle.dump(var_a, f)
                    var_a = "Epoch: %d Valid BPC: %.4f" % (i + 1, valid_perplexity)
                    pickle.dump(var_a, f)
...
# Following line means current file is executed under a shell instead of imported as a module.
if __name__ == "__main__":
    # Handle flags and command line args & Call current file's main function
    tf.app.run()
